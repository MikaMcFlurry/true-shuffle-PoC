...
| G5 Hardening | Testmatrix, Security/Privacy, Evidence Matrix, Release-Review | **TEILWEISE (automatisiert) / Live BLOCKED** | ERR-01…08 + MAN-01…05 abgedeckt (`ERR_MAN_COVERAGE.md`, 36 Tests, 4 Funde gefixt); unabhängiger Security-Review (20 Findings, `SECURITY_PRIVACY_STATUS.md`) — alle 5 blockierenden Auflagen behoben+getestet (`07d5575`), Rest als Restrisiko dokumentiert; Disconnect-Löschpfad F10 dreistufig implementiert+getestet (`retention.py`); 10k-Performance-Profil (Replan 54,6 s→1,4 s, `PHASE4_PERF_PROFILE.md`); Concurrency/Locking + Flakiness-Wurzel behoben; Policy-/Kommerz-Status (`POLICY_COMMERCIAL_STATUS.md`). **Offen für PASS:** unabhängiger Release-Review (Gesamt), Abschlussbericht. Suiten: **704 Unit/API + 54 Browser + 8 slow grün, Ruff clean.** Live-Zeilen BLOCKED. |
